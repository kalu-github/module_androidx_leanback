...
//    protected boolean isHideDefaultData() {
//        return true;
//    }
    ...
    public void onBindViewHolder(ViewHolder viewHolder, Object object) {
        ...
        try {
            ...
            // ??
            setPlayingRange(viewHolder.view, 0);
            setPlayingEpisode(viewHolder.view, 0, -1, false);
//            // 默认数据隐藏
//            if (isHideDefaultData()) {
...
//                throw new Exception("length warning: " + length);
            ...
            for (int i = 0; i < episodeNum; i++) {
                ...
                child.setOnKeyListener(new View.OnKeyListener() {
                    ...
                    public boolean onKey(View view, int keyCode, KeyEvent keyEvent) {
                        // left
                        if (keyEvent.getAction() == KeyEvent.ACTION_DOWN && keyCode == KeyEvent.KEYCODE_DPAD_LEFT) {
                            return dispatchEventEpisode(view, View.FOCUS_LEFT);
                        }
                        // right
                        else if (keyEvent.getAction() == KeyEvent.ACTION_DOWN && keyCode == KeyEvent.KEYCODE_DPAD_RIGHT) {
                            return dispatchEventEpisode(view, View.FOCUS_RIGHT);
                        }
                        // up
                        else if (keyEvent.getAction() == KeyEvent.ACTION_DOWN && keyCode == KeyEvent.KEYCODE_DPAD_UP) {
    ...
    private int getEpisodeCurrentPlayingIndexOfChild(View viewGroup) {
        try {
            ...
            LeanBackUtil.log("ListTvEpisodesDoubleRowPresenter => getEpisodeCurrentPlayingIndexOfChild => " + e.getMessage());
            return -1;
        }
    }
    ...
    private boolean dispatchEventEpisode(@NonNull View focusView,
        ...
        try {
            ...
            if (!(episodeGroup instanceof LinearLayout))
                throw new Exception("episodeGroup error: not instanceof LinearLayout");
            int episodeChildCountReal = episodeGroup.getChildCount();
            ...
            // left-fail
            if (direction == View.FOCUS_LEFT && episodeIndex <= 0) {
                return true;
            }
            // left-move
            else if (direction == View.FOCUS_LEFT && indexOfChild > 0) {
    ...
    public void checkedPlayingPosition(@NonNull View viewGroup,
        ...
        try {
            ...
            setPlayingEpisode(viewGroup, checkedEpisodeIndex, playingIndexOfChild, true);
        } catch (Exception e) {
