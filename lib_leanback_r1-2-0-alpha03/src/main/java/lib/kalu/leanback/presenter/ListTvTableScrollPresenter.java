...
public abstract class ListTvTableScrollPresenter<T extends TvEpisodesGridItemBean> extends Presenter implements ListTvPresenterImpl {
    ...
    public ViewHolder onCreateViewHolder(ViewGroup parent) {
        try {
            ...
            RootLinearLayout inflate = (RootLinearLayout) LayoutInflater.from(context).inflate(R.layout.lb_list_tv_table_scroll, parent, false);
            // listener
            resistOnVisibilityChangedListener2(inflate);
                        ...
                        public void onClick(View v) {
                            try {
                                ...
                                if (playing) {
                                    ...
                                    ((T) tag).setChecked(true);
                                    onBindHolder(v.getContext(), v, ((T) tag), episodeIndex);
                                    onClickHolder(v.getContext(), v, ((T) tag), episodeIndex, playingPosition, true);
                                }
                            } catch (Exception e) {
                        ...
                        public boolean onKey(View view, int i, KeyEvent keyEvent) {
                            ...
                            if (keyEvent.getAction() == KeyEvent.ACTION_DOWN && keyEvent.getKeyCode() == KeyEvent.KEYCODE_DPAD_LEFT) {
                                try {
                                    ...
                                    updateData(context, viewGroup, start, view, -1, false, playingPosition);
                                } catch (Exception e) {
...
//                throw new Exception("not empty");
    ...
    private void updateData(Context context, View viewGroup, int startPosition, View focusView, int checkIndex, boolean isFromUser, int playingPosition) {
        try {
            int column = initColumn();
            int row = initRow();
            if (column <= 0 || row <= 0)
            ...
            for (int n = 0; n < row; n++) {
                ...
                for (int m = 0; m < columnReal; m++) {
                    ...
                    if (index + 1 > size) {
                        ...
                        if (checkIndex != -1 && t.getEpisodeIndex() == checkIndex) {
                            onClickHolder(view.getContext(), view, t, checkIndex, playingPosition, false);
                        }
                    }
                }
            }
...
//    }
    ...
    public void checkedPlayingPosition(@NonNull View viewGroup, @NonNull int checkedIndex) {
        try {
            ...
            updateData(viewGroup.getContext(), viewGroup, startIndex, focusView, checkedIndex, true, playingPosition);
        } catch (Exception e) {
