                        ...
                        public boolean onKey(View view, int i, KeyEvent keyEvent) {
                            ...
                            if (keyEvent.getAction() == KeyEvent.ACTION_DOWN && keyEvent.getKeyCode() == KeyEvent.KEYCODE_DPAD_LEFT) {
                                try {
                                    ...
                                    updateData(context, viewGroup, start, view);
                                } catch (Exception e) {
...
//                throw new Exception("not empty");
    ...
    private void updateData(Context context, View viewGroup, int startPosition, View focusView) {
        try {
            int column = initColumn();
            int row = initRow();
            if (column <= 0 || row <= 0)
            ...
            for (int n = 0; n < row; n++) {
                ...
                for (int m = 0; m < columnReal; m++) {
                    ...
                    if (index + 1 > size) {
                        ...
                        T t = mData.get(index);
                        if (null != focusView && view == focusView) {
                            t.setFocus(true);
                        } else {
                            t.setFocus(false);
                        }
